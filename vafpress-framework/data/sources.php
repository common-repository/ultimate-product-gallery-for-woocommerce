...
VP_Security::instance()->whitelist_function('upgfw_vp_dep_scrollbar_theme_has_buttons');

function upgfw_vp_dep_scrollbar_theme_has_buttons($value)
{
	if( $value == 11 ) {
		return false;
	} else {
...
VP_Security::instance()->whitelist_function('upgfw_vp_dep_lightbox_zoom_mode_1');

function upgfw_vp_dep_lightbox_zoom_mode_1($value)
{
	if( $value == 1 ) {
		return true;
	} else {
